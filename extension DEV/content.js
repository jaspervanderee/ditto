...
(function() {
  ...
  function extract() {
    ...
    // YouTube channel page detection (before single-video check)
    if (hostname.includes('youtube.com')) {
      const isChannelVideosPage = /^\/@[^/]+\/videos/.test(path) ||
                                   /^\/(channel|c|user)\/[^/]+\/videos/.test(path);
      const isChannelPage = /^\/@[^/]+/.test(path) ||
                            /^\/(channel|c|user)\/[^/]+/.test(path);

      if (isChannelVideosPage) {
        return Promise.resolve({ type: 'channel' });
      } else if (isChannelPage && !path.includes('/watch')) {
        return Promise.resolve({ type: 'channel_not_videos' });
      }
